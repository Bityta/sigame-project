...
output "connection_info" {
  ...
  value       = <<-EOT
    ...
    Monitoring:
      ...
      Prometheus:  http://${yandex_compute_instance.app_server.network_interface[0].nat_ip_address}:9090
    
    Next Steps:
      1. SSH to server
      2. cd /opt/sigame && sudo docker compose up -d
      3. Open Grafana at http://${yandex_compute_instance.app_server.network_interface[0].nat_ip_address}:3000
    ========================================
  EOT
}
